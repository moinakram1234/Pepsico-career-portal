...
export default function Home() {
  ...
  const handleCheckboxChange = (value, id, cv) => {
    let updatedRows;

    if (selectedRows.some((row) => row.id === id)) {
      updatedRows = selectedRows.filter((row) => row.id !== id);
    } else {
      updatedRows = [...selectedRows, { id, cv }];
    }

    setSelectedRows(updatedRows);

    // Ensure the selectAllClass checkbox exists before trying to set its checked property
    const selectAllCheckbox = document.querySelector(".selectAllClass");
    if (selectAllCheckbox) {
      selectAllCheckbox.checked = updatedRows.length === filteredData.length;
    }
  };
  ...
  const handleSelectAll = (value, data) => {
    ...
    if (value) {
      setSelectedRows(data.map((row) => ({ id: row._id, cv: row.cv })));
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const downloadZip = async () => {
    if (Array.isArray(selectedRows) && selectedRows.length > 0) {
      const filesArray = selectedRows.map((row) => ({
        path: row.cv, // The path of the file to download
        id: row.id, // The new filename (ID) for the file
      }));
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = "Cvs.zip"; // The name of the zip file
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
      } else {
        console.error("Failed to download files:", await response.text());
      }
    }
  };
      ...
      <div>
        <div className="container mx-auto p-4  ">
          <div className="flex justify-center mt-4 ">
            <h1
              className={`${styles.headertext} text-2xl font-bold  text-[#755E02] mb-10`}
            >
          ...
          {!selectedUser ? (
            <div>
              <div className="flex justify-between mb-5">
                ...
                <div>
                  <button
                    className="p-1 bg-[#ffcc00] text-sm text-[#755E02] rounded"
                  ...
                  <button
                    ...
                    className="p-1 bg-[#ffcc00] text-sm m-2 text-[#755E02] rounded"
                  >
              ...
              <div>
                ...
                {selectedRows.length > 0 && (
                  <div className="flex bg-[#EFBF00] p-2 rounded-lg  gap-5 mb-2">
                    <div>
                      <input
                        type="checkbox"
                        className="selectAllClass mt-2 "
                        onChange={(e) =>
                          handleSelectAll(e.target.checked, filteredData)
                        }
                      />{" "}
                      <span className="text-sm text-[#755E02] font-bold font-spartan mt-2">
                        Select All
                      </span>
                    </div>
                    <button
                      onClick={reject_selected_app}
                      className="p-2 shadow-sm border rounded-lg"
                    >
                      <div className="flex">
                    ...
                    <button
                      ...
                      className=" shadow-sm border p-2 rounded-lg"
                    >
                      <div className="flex">
